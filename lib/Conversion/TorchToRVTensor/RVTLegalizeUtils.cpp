...
#include "rvtensor/RVTensor/RVTensorUtils.h"
#include "torch-mlir/Conversion/TorchToRVTensor/RVTLegalizeCommon.h"

namespace mlir {
namespace rvtensor {

// Create a RVTENSOR rescale op from input framework tensor, 
// qpoints and rounding mode
Value buildRescale(PatternRewriter &rewriter, Operation *op,
                   ShapedType output_type, Value input_val, double scale,
                   int64_t in_qpoint, int64_t out_qpoint, bool double_round,
                   bool scale32) {
  int32_t multiplier;
  int32_t shift;
  ...
  auto rescale_op = CreateOpAndInfer<rvtensor::RescaleOp>(
      rewriter, op->getLoc(), output_type, input_val,
      rewriter.getI32IntegerAttr(static_cast<int32_t>(in_qpoint)),
      rewriter.getI32IntegerAttr(static_cast<int32_t>(out_qpoint)),
...
// Creates RVTENSOR rescale op with int32 output
Value buildRescaleToInt32(PatternRewriter &rewriter, Operation *op,
  ...
  return buildRescale(rewriter, op, output_type, input_val, input_scale,
                      in_qpoint, 0, false, true);
}

// Creates a RVTENSOR rescale op based on conv2d parameters.
Value buildRescaleOpConvOutput(PatternRewriter &rewriter, Operation *op,
  ...
  int64_t out_qpoint = output_qtype.getZeroPoint();
  ...
  if (auto weight_per_tensor_qtype =
    ...
    auto rescale_op = CreateOpAndInfer<rvtensor::RescaleOp>(
        ...
        rewriter.getI32IntegerAttr(0), rewriter.getI32IntegerAttr(out_qpoint),
        rewriter.getDenseI32ArrayAttr({multiplier}),
        rewriter.getDenseI8ArrayAttr({static_cast<int8_t>(shift)}),
...
Value getRVTensorConstTensorSingleF32(PatternRewriter &rewriter, Operation *op,
                                  float val) {
  auto const_type = RankedTensorType::get({}, rewriter.getF32Type());
  auto const_attr = DenseElementsAttr::get(const_type, val);

  auto const_op =
      rewriter.create<rvtensor::ConstOp>(op->getLoc(), const_type, const_attr);
...
std::optional<Value> getZerosLikeTensor(PatternRewriter &rewriter,
  ...
  return CreateOpAndInfer<rvtensor::ConstOp>(rewriter, op->getLoc(), zeroType,
...
std::optional<Value> getConstTensor(PatternRewriter &rewriter, Operation *op,
  ...
  if (dtype) {
    return rewriter.createOrFold<rvtensor::CastOp>(
...
// Template specialization for float
LogicalResult rvtensorCastTensorToType(PatternRewriter &rewriter, Operation *op,
  ...
  if (destElemTy.isInteger(1)) {
    ...
    if (srcElemTy.isInteger(64)) {
      ...
      constOp =
          rvtensor::getConstTensor<int64_t>(rewriter, op, values, srcShape).value();
    } else if (srcElemTy.isInteger(32)) {
      SmallVector<int32_t> values(num_total_elements, 0);
      constOp =
          rvtensor::getConstTensor<int32_t>(rewriter, op, values, srcShape).value();
    } else if (srcElemTy.isF32()) {
      SmallVector<float> values(num_total_elements, 0.0);
      constOp =
          rvtensor::getConstTensor<float>(rewriter, op, values, srcShape).value();
    } else if (srcElemTy.isInteger(8)) {
      SmallVector<int8_t> values(num_total_elements, 0);
      constOp =
          rvtensor::getConstTensor<int8_t>(rewriter, op, values, srcShape).value();
    }
    Value equalToZero = rewriter.create<rvtensor::EqualOp>(op->getLoc(), destType,
                                                       src, constOp.value());
    result = rewriter.create<rvtensor::LogicalNotOp>(op->getLoc(), destType,
                                                 equalToZero);
  } else {
    result = rewriter.create<rvtensor::CastOp>(op->getLoc(), destType, src);
  }
  return success();
}
...
Value promoteType(PatternRewriter &rewriter, Value input, TensorType outType) {
  ...
  if (inType.getElementType() != outType.getElementType()) {
    ...
    return rewriter.create<rvtensor::CastOp>(op->getLoc(), promotedType, input);
  }
  return input;
}
...
LogicalResult getAvgPool2dAccType(PatternRewriter &rewriter, Value input,
  ...
  // RVTensor supports FP16 and FP32 accumulator type for FP16 input. When the time
